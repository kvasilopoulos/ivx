function f_minus = pvalue_integrand_minus_internal(rtilde_b,ttilde_gg,r_g,r_bb,r_gg,z_bb,rhobarhat,k)
...
exponent_minus_real = -(r_b.^2)./(2.*r_bb) + ReMatrix_11.*q_g - ReMatrix_1221.*sqrt(q_g).*z_bb + ReMatrix_22.*(z_bb.^2);

exponent_minus_imag = log(1-ttilde_gg).*r_gg + ImMatrix_11.*q_g - ImMatrix_1221.*sqrt(q_g).*z_bb + ImMatrix_22.*(z_bb.^2);

f_minus = exp(exponent_minus_real) .* (Multiplier_real .* cos(exponent_minus_imag) - Multiplier_imag .* sin(exponent_minus_imag)) ...
