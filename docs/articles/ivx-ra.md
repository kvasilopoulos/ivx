...
\
`m`` ``<-`` `[`ivx_ra`](https://kvasilopoulos.github.io/ivx/reference/ivx_ra.md)`(``Ret`` ``~`` ``DP`` ``+`` ``TBL``, data ``=`` ``kms``)`\
`m`\
`#> `\
`#> Call:`\
...
\
[`rbind`](https://rdrr.io/r/base/cbind.html)`(``ivx ``=`` `[`coef`](https://rdrr.io/r/stats/coef.html)`(`[`ivx`](https://kvasilopoulos.github.io/ivx/reference/ivx.md)`(``Ret`` ``~`` ``DP`` ``+`` ``TBL``, data ``=`` ``kms``)``)``, ivx_ra ``=`` `[`coef`](https://rdrr.io/r/stats/coef.html)`(``m``)``)`\
`#>                  DP         TBL`\
...
- Short horizon only, and no bootstrap:
  [`ivx_boot()`](https://kvasilopoulos.github.io/ivx/reference/ivx_boot.md)
  does not accept `ivx_ra` objects.
