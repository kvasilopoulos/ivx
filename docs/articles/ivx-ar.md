...
\
`m`` ``<-`` `[`ivx_ar`](https://kvasilopoulos.github.io/ivx/reference/ivx_ar.md)`(``hpi`` ``~`` `[`log`](https://rdrr.io/r/base/Log.html)`(``res``)`` ``+`` ``cpi``, data ``=`` ``ylpc``)`\
`m`\
`#> `\
`#> Call:`\
...
[`coef`](https://rdrr.io/r/stats/coef.html)`(`[`ivx_ar`](https://kvasilopoulos.github.io/ivx/reference/ivx_ar.md)`(``hpi`` ``~`` `[`log`](https://rdrr.io/r/base/Log.html)`(``res``)`` ``+`` ``cpi``, data ``=`` ``ylpc``, ar ``=`` ``1``)``)`\
`#>      log(res)           cpi `\
...
- `ar = "forecast"` delegates order selection to
  `forecast::auto.arima()` and needs that package.
