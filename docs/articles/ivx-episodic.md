...
\
`mod`` ``<-`` `[`ivx`](https://kvasilopoulos.github.io/ivx/reference/ivx.md)`(``Ret`` ``~`` ``DP``, data ``=`` ``kms``)`\
`e`` ``<-`` `[`ivx_episodic`](https://kvasilopoulos.github.io/ivx/reference/ivx_episodic.md)`(``mod``, scheme ``=`` ``"rolling"``, window ``=`` ``0.2``, B ``=`` ``499``, seed ``=`` ``1``)`\
`e`\
`#> `\
`#> Call:`\
...
\
[`head`](https://rdrr.io/r/utils/head.html)`(``e``$``sequence``, ``3``)`\
`#>   start end      Wald      t_DP`\
`#> 1     2 207 0.3000535 0.5477714`\
`#> 2     3 208 0.2980866 0.5459731`\
`#> 3     4 209 0.3001673 0.5478752`\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``kms``$``Date``[``e``$``sequence``$``end``]``, ``e``$``sequence``$``t_DP``, type ``=`` ``"l"``,`\
`     xlab ``=`` ``"window end"``, ylab ``=`` ``"subsample IVX t-ratio"``, main ``=`` ``"Rolling windows, 20% of the sample"``)`\
[`abline`](https://rdrr.io/r/graphics/abline.html)`(``h ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``-``1.96``, ``1.96``)``, lty ``=`` ``2``)`
...
\
[`ivx_episodic`](https://kvasilopoulos.github.io/ivx/reference/ivx_episodic.md)`(``mod``, scheme ``=`` ``"forward"``, window ``=`` ``0.2``, B ``=`` ``199``, seed ``=`` ``1``)``$``p.value`\
`#>       sup       inf    sup_sq `\
`#> 0.2462312 0.7788945 0.4371859`\
[`ivx_episodic`](https://kvasilopoulos.github.io/ivx/reference/ivx_episodic.md)`(``mod``, scheme ``=`` ``"backward"``, window ``=`` ``0.8``, B ``=`` ``199``, seed ``=`` ``1``)``$``p.value`\
`#>        sup        inf     sup_sq `\
