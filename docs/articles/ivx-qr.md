...
\
`m`` ``<-`` `[`ivx_qr`](https://kvasilopoulos.github.io/ivx/reference/ivx_qr.md)`(``Ret`` ``~`` ``DP``, data ``=`` ``kms``, tau ``=`` ``0.5``)`\
[`summary`](https://rdrr.io/r/base/summary.html)`(``m``)`\
`#> `\
`#> Call:`\
...
\
`ms`` ``<-`` `[`ivx_qr`](https://kvasilopoulos.github.io/ivx/reference/ivx_qr.md)`(``Ret`` ``~`` ``DP`` ``+`` ``TBL``, data ``=`` ``kms``, tau ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``0.1``, ``0.25``, ``0.5``, ``0.75``, ``0.9``)``)`\
[`t`](https://rdrr.io/r/base/t.html)`(`[`sapply`](https://rdrr.io/r/base/lapply.html)`(``ms``, ``function``(``f``)`` `[`c`](https://rdrr.io/r/base/c.html)`(``tau ``=`` ``f``$``tau``, `[`coef`](https://rdrr.io/r/stats/coef.html)`(``f``)``, Wald ``=`` `[`round`](https://rdrr.io/r/base/Round.html)`(``f``$``Wald_Joint``, ``2``)``,`\
`                            p ``=`` `[`round`](https://rdrr.io/r/base/Round.html)`(``1`` ``-`` `[`pchisq`](https://rdrr.io/r/stats/Chisquare.html)`(``f``$``Wald_Joint``, ``2``)``, ``3``)``)``)``)`\
`#>       tau           DP         TBL  Wald     p`\
...
\
[`ivx_qr`](https://kvasilopoulos.github.io/ivx/reference/ivx_qr.md)`(``Ret`` ``~`` ``DP``, data ``=`` ``kms``, tau ``=`` ``0.5``, beta ``=`` ``0.8``)``$``Wald_Joint`\
`#> [1] 0.005426976`
